 ...
 * @return array
 */
function myne_body_classes( $classes ) {
...
add_filter( 'body_class', 'myne_body_classes' );
 ...
 * @return array
 */
function myne_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'myne_post_classes', 10, 3 );
 ...
 * @return void
 */
function myne_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'myne_pingback_header' );
 ...
 * @return void
 */
function myne_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'myne_supports_js' );
 ...
 * @return array
 */
function myne_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'myne_comment_form_defaults' );
 ...
 * @return bool
 */
function myne_can_show_post_thumbnail() {
	/**
 ...
 * @since MYNE 1.0
 */
function myne_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Post title. Only visible to screen readers. */
		esc_html__( 'Continue reading %s', 'myne' ),
 ...
 * @since MYNE 1.0
 */
function myne_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . myne_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'myne_continue_reading_link_excerpt' );
 ...
 * @since MYNE 1.0
 */
function myne_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . myne_continue_reading_text() . '</a></div>';
	}
}

// Filter the content more link.
add_filter( 'the_content_more_link', 'myne_continue_reading_link' );

if ( ! function_exists( 'myne_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function myne_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'myne' ) : $title;
	}
}
add_filter( 'the_title', 'myne_post_title' );
 ...
 * @return string
 */
function myne_get_icon_svg( $group, $icon, $size = 24 ) {
	return myne_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function myne_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? myne_get_icon_svg( 'ui', 'arrow_right' ) : myne_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? myne_get_icon_svg( 'ui', 'arrow_left' ) : myne_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'myne_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function myne_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'myne_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'myne_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'myne_generate_css' ) ) {
	...
	return myne_generate_css( // @phpstan-ignore-line.
 ...
 */
function myne_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 *
 * @since MYNE 1.0
 * @since MYNE 1.4 Corrected parameter name for `$output`,
 ...
 */
function myne_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'myne' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'myne' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" spellcheck="false" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'myne' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'myne' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'myne_password_form', 10, 2 );
 ...
 */
function myne_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'myne_get_attachment_image_attributes', 10, 3 );
