...
class myne_Dark_Mode {
	...
	public function __construct() {
		...
		// Add HTML classes.
		add_filter( 'myne_html_classes', array( $this, 'html_classes' ) );
	...
	public function editor_custom_color_variables() {
		...
		if ( $should_respect_color_scheme && myne_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
		...
		wp_enqueue_script(
			'myne-editor-dark-mode-support',
			get_template_directory_uri() . '/assets/js/editor-dark-mode-support.js',
			array( 'myne-dark-mode-support-toggle' ),
			'1.0.0',
			array( 'in_footer' => true )
		);
	}
	...
	public function customize_controls_enqueue_scripts() {
		...
		wp_enqueue_script(
			'myne-customize-controls',
			get_template_directory_uri() . '/assets/js/customize.js',
			array( 'customize-base', 'customize-controls', 'underscore', 'jquery', 'myne-customize-helpers' ),
			'1.0.0',
			array( 'in_footer' => true )
		);
	}
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', 'myne' );
		}
		...
		$wp_customize->add_control(
			new myne_Customize_Notice_Control(
				$wp_customize,
				'respect_user_color_preference_notice',
				array(
					'section'         => 'colors',
					...
					'active_callback' => static function () {
						return 127 >= myne_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) );
					},
				)
			)
		);
		...
		$description  = '<p>';
		$description .= sprintf(
			/* translators: %s: MYNE support article URL. */
			__( 'Dark Mode is a device setting. If a visitor to your site requests it, your site will be shown with a dark background and light text. <a href="%s">Learn more about Dark Mode.</a>', 'myne' ),
			esc_url( __( 'https://wordpress.org/documentation/article/twenty-twenty-one/#dark-mode-support', 'myne' ) )
		);
		$description .= '</p>';
		$description .= '<p>' . __( 'Dark Mode can also be turned on and off with a button that you can find in the bottom corner of the page.', 'myne' ) . '</p>';
		...
		$wp_customize->add_control(
			...
			array(
				'type'            => 'checkbox',
				'section'         => 'colors',
				'label'           => esc_html__( 'Dark Mode support', 'myne' ),
				'priority'        => 110,
	...
	public function html_classes( $classes ) {
		...
		$background_color            = get_theme_mod( 'background_color', 'D1E4DD' );
		$should_respect_color_scheme = get_theme_mod( 'respect_user_color_preference', false );
		if ( $should_respect_color_scheme && 127 <= myne_Custom_Colors::get_relative_luminance_from_hex( $background_color ) ) {
	...
	public function admin_body_classes( $classes ) {
		...
		if ( $current_screen->is_block_editor() ) {
			...
			if ( $should_respect_color_scheme && myne_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
				$classes .= ' myne-supports-dark-theme';
			}
		}
	...
	public function switch_should_render() {
		...
		return (
			...
			! $is_IE &&
			127 <= myne_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) )
		);
	}
	...
	public function the_html( $attrs = array() ) {
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', 'myne' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', 'myne' ); ?>';
			}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( 'MYNE uses LocalStorage when Dark Mode support is enabled.', 'myne' ) . '</p>'
				. '<strong class="privacy-policy-tutorial">' . __( 'Suggested text:', 'myne' ) . '</strong> '
				. __( 'This website uses LocalStorage to save the setting when Dark Mode support is turned on or off.<br> LocalStorage is necessary for the setting to work and is only used when a user clicks on the Dark Mode button.<br> No data is saved in the database or transferred.', 'myne' );
		wp_add_privacy_policy_content( __( 'MYNE', 'myne' ), wp_kses_post( wpautop( $content, false ) ) );
	}
}
